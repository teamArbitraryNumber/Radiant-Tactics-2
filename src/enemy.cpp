...
void Enemy::move(GameMap& map) {
    ...
    if (!player) {
        cout << "Player not found!" << endl; // Debug
        return; // Player not found, do nothing
    }
    ...
    if (!found) {
        cout << "No valid path to player!" << endl; // Debug
        return; // No valid path to player
    }
    ...
    cout << enemyType << " moved to (" << previous.first << ", " << previous.second << ")" << endl; // Debug
}
